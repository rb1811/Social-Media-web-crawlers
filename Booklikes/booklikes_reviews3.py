...
g3 = open('booklikes_reviews3.json', 'r')
for line in g3:
	try:
		done_urls[json.loads(line[:-2]).keys()[0]] = None
	except:
...
urlStr = []
reamining_book_list=[]
for urlStr in book_list[80000:]:
	if urlStr in done_urls:
		continue
	reamining_book_list.append(urlStr)

for urlStr in tqdm(reamining_book_list):
	...
	if len(booklikes_reviews)!=0:
		...
		# print "@@@@@@@@@@@@@@@@@@@@@@@@@"
		f = open('booklikes_reviews3.json','a')
	...
	while True:
		...
		# if 'Community Reviews' in str(soup.find('h1', attrs={'class':'set-clearfix'})):
			...
			# break
		soup_content = soup.find('div', attrs={'class':'container content'})
